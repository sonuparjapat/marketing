...
export default async function RefundPolicyPage() {
  const [page, settings] = await Promise.all([getPage('refund-policy'), getPublicSettings()]);
  if (!page) notFound();
      ...
      <div className="mx-auto max-w-[720px]">
        ...
        {(settings.agency_legal_name || settings.notice_period) && (
          <div className="mt-12 border-t border-line pt-6 text-sm text-faint">
            {settings.agency_legal_name && <p>Issued by {settings.agency_legal_name}.</p>}
            {settings.notice_period && <p className="mt-1">Cancellation notice period: {settings.notice_period}.</p>}
          </div>
        )}
      </div>
    </main>
