...
<li>Premium content/feature gating for customer accounts — the auth system and <code>customers.is_premium</code> flag exist, but no admin page manages "premium" status, no content is marked premium-only, and no paywall/upgrade UI exists. Build this once the actual premium offering is defined, rather than guessing at a shape now — see the Customer accounts change history entry above</li>
<li>Customer account management in the admin panel (no <code>/admin/customers</code> page) — customers aren't part of the RBAC permission catalog either, since there's nothing yet for an admin to do with them beyond what direct DB access covers</li>
</ul>

<h2>Change history</h2>
<p>High-level phases this project has gone through, newest first:</p>
<ul>
<li><strong>Customer accounts (public site)</strong> — a new, entirely separate identity space from admins: <code>customers</code> table (id, name, email, password_hash, is_premium, is_active), <code>backend/src/modules/customerAuth/</code> (register/login/logout/me), and a new <code>customerAuth</code> middleware (<code>backend/src/middleware/auth.js</code>) reading a distinct <code>customer_token</code> cookie — not <code>token</code>, which admin sessions use — so one browser can hold an admin session and a customer session simultaneously without collision, and requires the JWT's <code>type</code> claim to be exactly <code>'customer'</code> (admin tokens carry no <code>type</code> claim, so they're rejected here even though both use the same <code>JWT_SECRET</code>). Rate-limited via a new <code>customerAuthLimiter</code> — deliberately a separate <code>express-rate-limit</code> instance from admin login's <code>authLimiter</code>, so heavy traffic on customer signups can't drain an admin's login attempt budget from the same IP or vice versa. Frontend: <code>CustomerAuthContext</code> (mirrors <code>AdminAuthContext</code>'s cached-then-refresh-in-background pattern via <code>GET /auth/me</code>), a separate <code>customerApiClient.ts</code> axios instance (attaches <code>customer_token</code>, and — unlike the admin client — a 401 does <em>not</em> force-navigate anywhere, since a logged-out visitor is still just a visitor), and <code>AuthModal.tsx</code> (combined login/register modal) wired into <code>Header.tsx</code> as a account icon (desktop) / "Sign in" row (mobile menu). <code>is_premium</code> is seeded as a placeholder flag only — nothing reads it yet, since no premium content or feature has been defined; deliberately not building speculative gating logic for a feature that doesn't exist yet (see "Explicitly out of scope" below).</li>
<li><strong>Mobile rich content rendering</strong> — <code>post/[slug].tsx</code> and <code>work/[slug].tsx</code> previously stripped all HTML to plain text via a regex (<code>stripHtml()</code>), so images/tables/YouTube embeds/formatting in blog and case-study content never reached the app. Considered <code>react-native-render-html</code> (a JS-to-native-component mapper) but it's unmaintained since Jan 2022 and fundamentally can't render an <code>&lt;iframe&gt;</code> (YouTube embed) or a real table layout — RN has no native table primitive. Went with <code>react-native-webview</code> instead (Expo SDK 57's officially recommended package, actively maintained, included in Expo Go) via a new <code>mobile/src/components/HtmlContent.tsx</code>: wraps the post/case-study HTML in a themed document (colors pulled from <code>useTheme()</code>) and renders it in a <code>WebView</code> with <code>scrollEnabled={false}</code> (the outer screen's <code>ScrollView</code> handles scrolling, not the WebView) sized dynamically via an injected script that posts <code>document.body.scrollHeight</code> back on load and on any <code>ResizeObserver</code>-detected change (images loading in changes the height after initial render). Added <code>mobile/.npmrc</code> with <code>legacy-peer-deps=true</code> to install it — the same class of peer-dependency conflict the frontend needed it for, caused here by expo-router's own bundled <code>@radix-ui</code>/<code>vaul</code> dependencies.</li>
<li><strong>Admin polish pass</strong> — Header.tsx (public nav) rebuilt with scroll-aware chrome, active-link underlines, and an animated mobile menu; AdminShell's sidebar is now <code>sticky</code> + full-viewport-height so it scrolls independently of page content instead of stretching with it (it had <code>overflow-y-auto</code> before but no constrained height, so it never actually triggered). RBAC gaps closed: fixed a stale <code>leads.view</code> string in <code>stats()</code> left over from the read/update rename (meant editors with real <code>leads.read</code> access saw the dashboard's lead data hidden); every custom (non-ResourceManager) admin page — PostEditor, Leads, Callbacks, Pages, Homepage Sections, Media, Settings — now calls <code>hasPermission()</code> to hide Save/Publish/Delete/toggle controls for admins who lack that action, instead of relying solely on the backend 403. Password management: Super Admin can reset any admin's password from Admin → Admins (<code>PATCH /admin/admins/:id/password</code>, no current-password check, gated <code>requireSuperAdmin</code>); any admin can change their own via a "Change password" link in the header (<code>PATCH /admin/change-password</code>, verifies current password first) — previously neither existed, only a direct DB edit. Pagination: added a shared <code>components/ui/Pagination.tsx</code>, wired into <code>ResourceManager</code> (20/page) and the Leads/Callbacks/Subscribers/Media (30/page)/Posts custom list pages, all of which previously fetched a flat 100–200 row cap with no way to reach anything past it; Posts' admin list also gained a dedicated <code>adminListPosts</code> handler (search + category filters server-side) replacing the generic <code>buildAdminCrud</code> list, since its filter UI needs to search across all posts, not just the loaded page. Services gained native HTML5 drag-and-drop reordering (<code>/admin/services</code> rebuilt as a dedicated page instead of a <code>ResourceManager</code> instance, calling the pre-existing but previously-unused <code>PATCH /admin/services/reorder</code>).</li>
...
<li><strong>Homepage engagement pass</strong> — fixed a real bug where <code>/admin/upload</code> silently stopped receiving files (a previous edit that added <code>checkPermission('media.create')</code> to that route accidentally dropped the <code>upload.single('file')</code> multer middleware from the same chain, so <code>req.file</code> was never populated and every upload — cover images, in-editor images — failed with "No file uploaded", surfaced to the user as a generic "Image upload failed" alert; fixed by restoring the multer middleware, and the frontend's catch-all alert now surfaces the real server message instead of a hardcoded string). Separately, added three reusable homepage components: <code>components/CountUp.tsx</code> (animates a stat's numeric portion from 0 when scrolled into view, via <code>framer-motion</code>'s <code>useSpring</code> — parses a leading prefix and trailing suffix like <code>+</code>/<code>%</code>/<code>x</code> off the value string so only the digits tween), <code>components/LogoMarquee.tsx</code> (CSS-only infinite scroll, pauses on hover, respects <code>prefers-reduced-motion</code>), and <code>components/TestimonialCarousel.tsx</code> (auto-rotating single-testimonial view with dot navigation, pauses on hover). The hero video and CTA banner also gained ambient drift/ken-burns CSS animations (<code>globals.css</code>: <code>.animate-drift</code>, <code>.animate-kenburns</code>, <code>.animate-marquee</code> — all reduced-motion-aware).</li>
